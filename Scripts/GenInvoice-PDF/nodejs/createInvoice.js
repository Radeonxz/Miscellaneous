...
exports.createInvoice = (invoice, contractor, path) => {
  ...
  for(let i = 0; i < pages; i++) {
    ...
    if(pages - i == 1) {
      ...
      generateInvoiceSubTotal(doc, invoice, taxes, invoiceTabHeight, invoiceTableTop, index);
    }
...
const generateHeader = (doc, contractor) => {
  doc
    ...
    .text(`${contractor.city}, ${contractor.province}, ${contractor.country}`, 200, 80, { align: 'right' })
...
const generateCustomerInformation = (doc, invoice, taxes, invoiceEdge) => {
  ...
  doc
    ...
    .text(`${invoice.shipping.city}, ${invoice.shipping.state}, ${invoice.shipping.country}, ${invoice.shipping.postalcode}`, 300, customerInformationTop + 30)
    .moveDown();

  generateHr(doc, 252);
};

const generateInvoiceTable = (doc, invoiceItems, invoiceTabHeight, invoiceTableTop) => {
  let i;
  // const invoiceTableTop = 330;

  doc.font('Helvetica-Bold');
  generateTableRow(doc, invoiceTableTop, 'Service', 'Description', 'Unit Cost', 'Quantity', 'Line Total');
  ...
  for (i = 0; i < invoiceItems.length; i++) {
    ...
    generateTableRow(doc, position, item.item, item.description, unitPrice, item.quantity, totalPrice);

    generateHr(doc, position + 20);
  }
};

const generateInvoiceSubTotal = (doc, invoice, taxes, invoiceTabHeight, invoiceTableTop, i) => {
  ...
  generateTableRow(doc, gstPosition, '', '', `GST: ${taxes.gstRate}%`, '', formatCurrency(taxes.gst));

  const qstPosition = gstPosition + 20;
  generateTableRow(doc, qstPosition, '', '', `QST: ${taxes.qstRate}%`, '', formatCurrency(taxes.qst));
  ...
  doc.font('Helvetica-Bold');
  const blanceDue = formatCurrency(invoice.subtotal + taxes.gst + taxes.qst - invoice.paid);
